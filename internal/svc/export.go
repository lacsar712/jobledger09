...
import (
	...
	"example.com/jobledger/internal/util/filedump"
	"example.com/jobledger/internal/util/jsonx"
)
...
func (c *Catalog) ExportJSON(ctx context.Context) ([]byte, error) {
	...
	exp := model.ExportFile{Name: "JobLedger As-Run", Age: len(recs)}
	...
	snap := Snapshot{
		...
		Records: recs,
		Export:  decoded,
		Meta:    map[string]string{"app": "jobledger"},
	}
